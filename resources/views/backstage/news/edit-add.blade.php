                                ...
                                <div class="form-group">
                                    ...
                                    @elseif($row->type == "select_dropdown")
                                        ...
                                        @if(isset($options->relationship))
                                            ...
                                            @endif
                                        @else
                                            @if($row->field == "news_category_id")
                                                <select class="form-control select2" name="news_category_id">
                                                    @foreach(App\NewsCategory::all() as $category)
                                                        <option value="{{ $category->id }}" @if(isset($dataTypeContent->news_category_id) && $dataTypeContent->news_category_id == $category->id){{ 'selected="selected"' }}@endif>{{ $category->title }}</option>
                                                    @endforeach
                                                </select>
                                            @else
                                                <?php $selected_value = (isset($dataTypeContent->{$row->field}) && !is_null(old($row->field, $dataTypeContent->{$row->field}))) ? old($row->field, $dataTypeContent->{$row->field}) : old($row->field); ?>
